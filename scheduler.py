...
def send_telegram_message(message):
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        print("⚠️ Telegram not configured yet. Skipping notification.")
        return
...
def extract_price_value(price_str: str) -> float:
    """Extract numeric value from price string like 'KSh 1,299' -> 1299.0"""
    try:
...
⏰ {datetime.now().strftime('%H:%M')}"""
                ...
                print(f"    ❌ Failed to connect to {site.name}")
    ...
    db.commit()
    db.close()
    print(f"✅ Scrape complete! {new_deals_count} new deals, {price_drops_count} price changes.\n")
